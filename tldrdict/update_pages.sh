...
curl -s 'https://github.com/tldr-pages/tldr/releases/latest/download/index.json'|  jq --compact-output '[.commands[] | {command: .name, info: .targets | group_by(.os)[] | {os: .[] | .os, languages: [.[] | .language]}}]  | unique' > commands2.json
...
curl -s 'https://github.com/tldr-pages/tldr/releases/latest/download/index.json' |  jq --compact-output '{supportedLanguages: [.commands[] | .language | .[]] | unique}' > temp.json && jq --compact-output --slurp add version.json temp.json | sponge version.json && rm temp.json
